...
Add optimization step in yosys flow:
```shell
opt_clean -purge
```
...
**Functionality:**  
2-to-1 multiplexer; outputs `1` when `a = 1`, otherwise `b`.
...
**Functionality:**
- D flip-flop always drives `q = 1` (regardless of input or clock).
...
## Summary
- **Focus:** Optimization techniques for combinational and sequential circuits.

- **Topics Covered:**
  1. **Constant Propagation** – simplify logic with constants.
  2. **State Optimization** – reduce FSM states and power.
  3. **Cloning** – duplicate logic cells for load balancing.
  4. **Retiming** – move registers to optimize performance.

- **Labs:** 6 Verilog-based labs showing simplifications and sequential optimizations.
